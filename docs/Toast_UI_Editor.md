...
기본 설정 언어는 `영어`입니다. *때문에, `en-us.js` 파일은 가져올 필요가 없습니다.*
...
### :hammer_and_wrench: 툴바 버튼 커스터마이징
...
### :hammer_and_wrench: 다음은 클릭 시 팝업 창을 띄우도록 하는 방법입니다.  
...
**기존에 제공되는 버튼**을 바탕으로 커스텀할 경우, `command` 옵션을 통해 `'blod'`를 연결하였습니다.  

하지만, **개인 커스텀 버튼**의 경우 직접 버튼 `요소`와 `기능`을 생성하여 `el 옵션에 지정`해야합니다.

<br>

### 3) 툴바 상태 변경

에디터에서 내용을 입력 시 **포커스된 노드**에 따라 `툴바의 상태도 함께 변화`하는데 이 툴바 상태도 조정할 수 있습니다.  

예를 들어, `**굵은** 글씨` 라는 내용이 에디터에 작성되어 있다면 저 `굵은` 이라는 단어는 마크다운에서는 `bold`가 적용되어 있다는 의미입니다.  

이때, `굵은`을 클릭하면 글자 사이에 커서가 깜빡이며 포커스가 되고 동시에 상단 툴바의 bold 버튼에 `active`라는 `class`가 추가됩니다.  

기본 설정은 active 클레스가 추가되면 파란색으로 색상이 변하도록 되어있습니다. 

이러한 툴바 스타일을 변경하고 싶다면 `CSS`에서 수정하면 되지만, 만약 툴바를 위에 설명한 내용처럼  커스텀 했다면 `'state'` 옵션을 추가하거나 `onUpdated()` 옵션을 지정해야합니다. 
<br>
...
***
기존 툴바를 사용할 경우 아래의 `state` 값을 사용해야만 툴바 요소의 **활성화 상태를 변경**할 수 있습니다.
...
에디터에 작성된 내용은 `Markdown`과 `HTML` 형태로 가져올 수 있습니다.  

`Markdow` 형식으로 가져오고 싶다면 `getMarkdown()` 메소드를 사용하면 되며, `HTML`은 `getHTML()`으로 가져오면 됩니다.  

❗ 토스트 에디터는 기본적으로 sanitizer를 제공하긴 하지만 생각보다 필터링 범주가 적기 때문에 개인적으로 필터링 함수를 적용시켜야합니다. 자세한 내용은 [여기](#white_check_mark-7-보안-필터링sanitizer)에서 확인 가능합니다.
...
그런데 왜 새로운 페이지를 만듬에도 불구하고 뷰어를 사용하지 않고 다시 에디터를 불러와 `factory()` 메소드를 사용했냐 하면, 뷰어에서는 **플러그인이 적용된 코드를 보여주는 기능이 없기 때문**에입니다.  
...
Toast UI Editor는 `마크다운(Markdown)` 모드와 `위지윅(WYSIWYG)` **두 모드를 함께 제공하는 만큼 이 둘 버전을 동기화 시켜주는 렌더러가 존재**합니다.  
...
이에 본 프로젝트에서는 **자체적인 보안 필터링을 적용**시켜 테스트를 진행하였습니다.  

보안 필터링 라이브러리는 토스트 에디터와 동일하게 `DOMPurify` 라이브러리를 사용하였으며, 몇 가지 옵션과 태그를 사용하여 보안을 강화시켰습니다.  
...
### :hammer_and_wrench: 수정 필요
저는 실제 사용하는 서버가 없고, `Toast Image Editor`를 사용하여 불러온 이미지를 편집까지 한 뒤 에디터에 포함시키고 싶었기 때문에 다음과 같은 로직을 만들었지만, **실제로 사용할 경우 로직의 수정이 필요**합니다.
